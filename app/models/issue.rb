...
class Issue < ActiveRecord::Base
  ...
  #aaaaaaaa
  def self.score_field
    @@score_field ||= CustomField.find(:first, :conditions => {:name => 'Оценка'})
  end

  def score
    return @score if @score
    return unless self.class.score_field
    cv = custom_values.find(:first, :conditions => {:custom_field_id => self.class.score_field.id})

    @score = cv ? cv.value.to_f : 0.0
  end
  
#  def self.avg_score(issue_ids)
#    return unless score_field
#    scores = CustomValue.find(
#      :all, 
#      :conditions => {
#        :customized_type => 'Issue', 
#        :custom_field_id => score_field.id, 
#        :customized_id => issue_ids}).map(&:value).map(&:to_f)
#
 ...
 def self.avg_score(issue_ids)
    ...
    scores = CustomValue.find(
      ...
      :conditions => {
        :customized_type => 'Issue',
        :custom_field_id => score_field.id,
        :customized_id => issue_ids}).map(&:value).map(&:to_f).delete_if{|value| value == 0.0}
